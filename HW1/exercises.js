...
var sum = function (arr) {
  ...
  for (var i = 0; i < arr.length; i++) {
    if (typeof(arr[i]) === 'number') {
      ...
      throw new Error('Number Expected Here!');
    }
  }
  return tempSum;
};
...
var remove = function (arr, item) {
  ...
  for (var i = 0; i < arr.length; i++) {
    if (arr[i] === item) {
      arr.splice(i, 1);
      // if we delete one element from the array, we need to turn the index i to i - 1;
      i--;
    }
  }
  return arr.slice();
...
var findDuplicates = function (arr) {
  ...
  for (var i = 1; i < arr.length; i++) {
    // if arr[i] is the last element, we can't check arr[i] !== arr[i + 1] to avoid ArrayOutOfBound error
    if (i === arr.length - 1) {
      if (arr[i] === arr[i - 1]) {
        result.push(arr[i]);
      }
...
var getTotalPrice = function () {
  var itemsInCart = Object.keys(this);
  var totalPrice = 0;
  var self = this;
  itemsInCart.forEach (function addNumber (value) {
    if (typeof self[value] == 'number' && self[value] > 0) {
      totalPrice += self.prices[value] * self[value];
    }
  })
  return totalPrice;
};

//cart.getTotalPrice = getTotalPrice.bind(cart);
...
var getSalePrice = function () {
  this.hasFlower = hasItemMixin('flower');
  if (this.hasFlower()) {
    return 0.9 * this.getTotalPrice();
  } else {
...
var reduce = function (array, reductionFunction, seedValue) {
  var aggregate = seedValue;
  for (var i = 0; i < array.length; i++) {
    aggregate = reductionFunction.apply(null,[aggregate].concat(array[i]));
  }
  return aggregate;
};
